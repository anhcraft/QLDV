  ...
  <div class="max-w-[1024px] m-auto pb-16 p-5 md:px-10">
    ...
    <LoadingState ref="loadingState">
      <header class="border-b-2 border-b-slate-300 pb-3 text-xl flex flex-col md:flex-row gap-2">
        ...
        <button class="btn-info mr-5"
                :class="{'opacity-50' : !event.contest.acceptingAnswers || this.contestSessions.length >= this.event.contest.limitSessions}"
                @click="joinContest" v-if="this.activeContestSession === undefined">Vào thi
        </button>
      </header>

      <div class="grid grid-cols-1 md:grid-cols-3">
        <div class="md:border-l-2 md:border-l-slate-300 pt-5 md:p-5 md:order-last"
             v-if="activeContestSession === undefined">
          ...
          <div v-else>
            <div class="mb-5" v-if="activeContestSession === undefined && currentContestSession !== undefined">
              <div class="border-l-4 border-l-green-300 bg-green-200 px-4 py-2 flex flex-row">
                <div class="grow">Thông tin bài làm #{{ selectedContestSession + 1 }} ({{
                    new Intl.DateTimeFormat("vi-VN", {
                      timeStyle: "medium",
                      dateStyle: "short"
                    }).format(new Date(currentContestSession.startTime))
                  }})
                </div>
                <div class="flex flex-row">
                  <div class="p-1 cursor-pointer hover:bg-green-100"
                       :class="{'opacity-20' : selectedContestSession === 0}"
                       @click="selectedContestSession = Math.max(0, selectedContestSession-1)">
                    <ChevronLeftIcon class="w-4 text-gray-600"></ChevronLeftIcon>
                  </div>
                  <div class="p-1 cursor-pointer hover:bg-green-100"
                       :class="{'opacity-20' : selectedContestSession === contestSessions.length-1}"
              ...
              <div class="flex flex-row place-items-center mt-3">
                <div class="grow text-center">
                  <span class="text-5xl">{{
                      currentContestSession.answerAccuracy.filter((v) => v === true).length
                    }}</span><span class="text-4xl"> / {{ currentContestSession.answerSheet.length }}</span>
                </div>
                <div class="text-sm">
                  <p>Điểm: {{ currentContestSession.score.toFixed(1) }}</p>
                  <p>Chính xác: {{ Math.floor(currentContestSession.score * 10) }}%</p>
                  <p>Thời gian: {{
                      this.stringifyTime(currentContestSession.lastAnswerSubmittedTime - currentContestSession.startTime)
                    }}</p>
                </div>
              </div>
            </div>
              ...
              <div class="mb-5 md:mb-10" v-for="(q, i) in currentContestSession.questionSheet">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-x-2 gap-y-1 mt-1">
                  <div class="flex flex-row gap-2 place-items-center" v-for="(c, j) in q.choices"
                       :class="getResultBackground(currentContestSession, i, j)">
                    <input type="radio" class="scale-125" :value="j" v-model="currentContestSession.answerSheet[i]"
                           :disabled="endingContest || activeContestSession === undefined">
      ...
      <div
          class="fixed top-20 md:top-1/2 right-0 md:right-10 md:-translate-y-1/2	p-5 md:p-8 bg-gray-200 shadow-lg shadow-slate-400"
          v-if="activeContestSession !== undefined">
        ...
        <div class="grid grid-cols-4 gap-1 place-items-center mt-10">
          <svg height="16" width="16" class="scale-75 cursor-pointer"
               v-for="(_, i) in activeContestSession.questionSheet" @click="scrollToQuestion(i)">
